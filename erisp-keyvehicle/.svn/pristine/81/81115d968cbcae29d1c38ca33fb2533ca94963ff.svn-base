app.controller('busMapCtrl', [
		...
		'SweetAlert',
		'$interval',
		function($scope, $modal, $filter, $timeout, $state, $stateParams, busMapService, $rootScope,SweetAlert, $interval) {
...
//	$scope.plateNumberLocations = ["B","C","D"];//车牌地址数组
    ...
    var layerOfBusText; //新能源图层
	var mapZoom = 16;
//	var mapZoom = 8;//内网地图缩放等级
	//标记当前图层是否在显示状态
	var layerOfBusFlag = false;
    ...
    $scope.queryFunction = function () {
    	...
    	busMapService.queryVehicleInfo($scope.queryConditionData,function (data) {
    	    if (data.state == 200) {
            	$scope.vehicleInfoList = data.messageBody.vehicleInfo;
            	console.log($scope.vehicleInfoList);
	...
	$scope.tabOnOneVehicle = function(plateNumber) {
	    ...
	    busMapService.queryOneVehicleRealTimeInfoSyn(queryConditionParams)
	    .then(function(data){
	    	if (data.state == 200 && data.messageBody.oneVehicleRealTimeInfo != null) {
		        ...
		        var onState = "";
		        var isRegister = "";
		        busMapService.queryVehicleInfoByVehicleTypeSyn(vehicleType, departName, pn, onState, isRegister)
		        .then(function(data) {
		        	...
		        	busMapService.queryDataOfDialogSyn(plateNumber, departName)
		            .then(function(data) {
		            	if (data.status == 0 && data.datas.length>0) {
			        	...
			        	if(vehicleRealInfo.longitude!="" && vehicleRealInfo.latitude!=""){
			        		layerOfBus.suspend();
	                        layerOfBusText.suspend();
	                        layerOfBus.clear();
	                        layerOfBusText.clear();
				            ...
				            if(vehicleRealInfo.gpsState==1){
				            	pic = new esri.symbol.PictureMarkerSymbol("assets/images/bus/bus-90.png", imageSize, imageSize);
				            }else{
				            	pic = new esri.symbol.PictureMarkerSymbol("assets/images/bus/bus-offline.png", imageSize, imageSize);
				            }
				            attr = {
		                    ...
		                    var title = "<div style='line-height:2;'><font style='font-weight:bold;font-size:18px;color:#000000;z-index:-1;'>" + 
		                    	"车牌号码：<label class='pn' style='font-size:18px;'>" + plateNumber + "</label>——公交车</font></div>" + 
		                    	"<div style='height:2px;border:none;border-top:1px solid #919191;width:98%;'></div>";
		                    var content = "<div class='row-fluid' style='border: 1px solid #ccc; width: 100%;height:90px;z-index:-1;'>" + 
...
//			                    "<tr><td colspan='2' style='font-size:14px;'><font style='font-weight:bold;font-size:14px;'>行业主管部门：</font>" + result.competentAuthority + "</td></tr>" + 
			                    ...
			                    "<tr><td colspan='2' style='font-size:14px;'><font style='font-weight:bold;font-size:14px;'>驾驶员：李云飞</font></td></tr>" + 
...
//            map.addLayer(layer1);
            ...
            layerOfBus = new GraphicsLayer();
            map.addLayer(layerOfBus);
            //添加点击事件
            layerOfBus.on("mouse-down",function(e){
     			$scope.showDialog(e);
            });
     		
            layerOfBusText = new GraphicsLayer();
            map.addLayer(layerOfBusText);
            layerOfBusText.setVisibility(false);
            map.on("zoom", function(e){
            	if(map.getZoom()>mapZoom){
            		layerOfBusText.setVisibility(true);
            	}else{
            		layerOfBusText.setVisibility(false);
            	}
            });
          ...
          //点击车辆图标弹出气泡
       	 $scope.showDialog = function(e){
       		 ...
       		 var vehicleInfo;
       		 var vehicleBaseInfo;
       		 busMapService.queryVehicleDriveInfo(plateNumber,function (data) {
      			if (data.state == 200) {
      				 vehicleBaseInfo = data.messageBody.vehicleInfo;
      				var plate = plateNumber.replace('川','');
      			busMapService.queryDataOfDialog(plate,departName,function (data) {
        			if (data.status == 0) {
        			...
        			var title = "<div style='line-height:2;'><font style='font-weight:bold;font-size:18px;color:#000000;z-index:-1;'>" +
	          			"车牌号码：<label class='pn' font-size:18px;>"+plateNumber+"</label>——公交车</font></div>" +
	          			"<div style='height:2px;border:none;border-top:1px solid #919191;width:98%;'></div>";
     				var content = "<div class='row-fluid' style='border: 1px solid #ccc; width: 100%;height:90px;z-index:-1;'>"+
	     				"<div class='col-md-12 font_black_famil'>"+
	    				"<div style='font-size: 18px;'>基本信息</div>"+
...
//	        			"<tr><td colspan='2' style='font-size: 14px;'><font style='font-weight:bold;font-size:14px;'>所属企业：</font>"+vehicleBaseInfo.ascriptionCompany+"</td></tr>" +
    ...
    $("body").on("click","#toRealLine",function(){
    	...
    	var params={plateNumber:plateNumber, longitude:longitude, latitude:latitude,
    			type:"busMap", typeCheckedName:typeCheckedName, rightCheckedName:rightCheckedName,pageType:pageType};
    ...
    $("body").on("click","#showPlatePic",function(){
	 ...
	 $scope.query = function(vehicleType){
		 ...
		 var pn = "";
		 busMapService.queryVehicleInfoByVehicleType(vehicleType,departName,pn,onlineStateCheck,isRegisterArea,function (data) {
			 if (data.status == 0) {
	 ...
	 function showLocation(type, result) {
         ...
         for(var i = 0;i<result.length;i++){
             ...
             var pic = $scope.showPicture(result[i].course,result[i].gpsState,"bus");
             //创建图片样式符合
	 ...
	 $scope.showPicture = function(course,gpsState,type){
		 ...
		 var pms = new esri.symbol.PictureMarkerSymbol("assets/images/bus/"+picUrl,imageSize,imageSize);
		 return pms;
	 }
	 ...
	 $scope.queryEarlyWarningInfoCounts = function(){
		 ...
		 busMapService.queryEarlyWarningInfoCounts($scope.queryConditionData.type,departName,isRegisterArea,function (data) {
				if (data.status == 0) {
     ...
     $scope.hideAllGraphics = function(){
    	 layerOfBus.setVisibility(false);
 		 layerOfBusText.setVisibility(false);
     }
     
     //显示所有车辆图层
     $scope.showAllGraphics = function(){
    	 layerOfBus.setVisibility(true);
     }
    ...
    $scope.mouseEvent(bayonetAddressDiv);
 	...
 	$("body").on("click","a.accidentTotal",function(){
 		...
 		busMapService.searchAccidentInfoNoPage($scope.queryConditionData,function (data) {
			 if (data.state == 200) {
 	...
 	$("body").on("click","a.illegalTotal",function(){
 		...
 		busMapService.searchIllegalInfoNoPage($scope.queryConditionData,function (data) {
			 if (data.state == 200) {
